...
public class StreamingClip : MonoBehaviour
{
    ...
    public void Seek(float timeInSeconds)
    {
        ...
        try
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Seek -> {timeInSeconds:F2}s (BASS)");
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            if (isHttp)
            {
                ...
                try
                {
                    ...
                    {
                        isRadio = true;
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Early radio detection: No known end + invalid BASS length");
                    }
                    else if (hasSmallEndPosition && hasInvalidBassLength)
                    {
                        isRadio = true;
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Early radio detection: Small buffer ({endPos} bytes) + invalid BASS length");
                    }
                }
                catch { }
            }
            ...
            try
            {
                ...
                if (pmeta != IntPtr.Zero)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(icy))
                    {
                        ...
                        {
                            isRadio = true;
                            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio confirmed: ICY metadata");
                        }
                        format = "icy";
                    }
                }
            ...
            if (!isRadio && isHttp)
            {
                try
                {
                    ...
                    {
                        isRadio = true;
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio detected: No known end + invalid BASS length");
                    }
                    else if (hasVerySmallEndPosition && hasInvalidBassLength)
                    {
                        isRadio = true;
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio detected: Very small buffer ({endPos} bytes) + invalid BASS length");
                    }
                    else if (hasSmallEndPosition && hasContinuousDownload && hasInvalidBassLength)
                    {
                        isRadio = true;
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio detected: Small buffer + continuous download + invalid length");
                    }
                }
                catch (Exception ex)
                {
    ...
    private void FinishStartStream(ReadResult read)
    {
        ...
        if (PluginConfig.ShowDebug.Value) Debug.Log("[StreamingClip] StartStream: finishing on main thread");
        ...
        if (read.IsRadio)
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Setting RadioStream mode - Confirmed radio stream");
        }
        else if (isUrl)
        {
            CurrentPlaybackMode = MusicPlayerMode.UrlFinite;
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Setting UrlFinite mode - Remote file with known length");
            ...
            try
            {
                ...
                if (endPos <= 0 && bassLength <= 0)
                {
                    ...
                    CurrentPlaybackMode = MusicPlayerMode.RadioStream;
                    read.IsRadio = true; 
                    if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Re-classified as RadioStream - URL with no detectable length");
                }
            }
            catch { }
        }
        else
        {
            // Local files
            CurrentPlaybackMode = MusicPlayerMode.UrlFinite;
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Setting UrlFinite mode - Local file");
        }
        ...
        if (read.IsRadio)
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio stream detected - using {dummyMinutes}-minute dummy clip");
        }
        else if (isUrlStream && looksOpusLike)
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Remote OGG/Opus stream - using {dummyMinutes}-minute dummy clip");
        }
        else if (isUrlStream && frames < _deviceRate * 60 * 5) // Less than 5 minutes
        {
            ...
            needsDummyClip = true;
            dummyMinutes = DummyMinutes; // 15 minutes
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Short URL stream ({frames / _deviceRate:F0}s) - using {dummyMinutes}-minute dummy clip");
        }
        ...
        if (needsDummyClip)
        {
            ...
            if (frames < desiredFrames)
            {
                ...
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Created {dummyMinutes}-minute dummy clip ({frames} frames) to prevent interruptions");
            }
        }
        ...
        if (PluginConfig.ShowDebug.Value)
        {
    ...
    private int GetClipFramesForStreamUsingRead(ReadResult read, int stream, int deviceRate)
    {
        ...
        if (read.IsRadio)
        {
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Radio stream - using minimum 1-hour clip frames");
        ...
        try
        {
            if (read.LengthBytes > 0)
            {
                ...
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Using ReadResult length: {secs:F2}s -> {frames} frames");
                return frames;
            }
            ...
            if (lenBytes > 0)
            {
                ...
                int frames = Math.Max(deviceRate, (int)(secs * deviceRate));
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Using BASS Channel length: {secs:F2}s -> {frames} frames");
                return frames;
            }
            ...
            if (end > 0)
            {
                ...
                int frames = Math.Max(deviceRate, (int)(secs * deviceRate));
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Using stream end position: {secs:F2}s -> {frames} frames");
                return frames;
            }
        }
        catch (Exception ex)
        ...
        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Unknown length - using 15-minute fallback");
    ...
    private void HandleDownloadedSync(int handle)
    {
        ...
        if (CurrentPlaybackMode == MusicPlayerMode.RadioStream)
        {
            if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Ignoring DownloadedSync for radio stream");
            return;
        }
        try
        {
            ...
            if (finalLenBytes > 0)
            {
                ...
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] DownloadedSync final length: {secs:F3}s (bytes={finalLenBytes})");
        ...
        try
        {
            ...
            if (_hasRecordedRealLength)
            {
                if (lenBytes > 0 && lenBytes != _initialPrescanLengthBytes)
                {
                    ...
                    TotalTime = (float)secs;
                    _hasRealLength = true;
                    if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] PollForLengthChange (post-recorded): authoritative length -> {secs:F3}s");
                }
                return;
            }
            ...
            if (_watchingForRealLength)
            {
                ...
                if (authoritativeLen > 0 && authoritativeLen != _initialPrescanLengthBytes)
                {
                    ...
                    if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Prescan diverged -> recorded real length = {secs:F3}s (bytes={authoritativeLen}). DownloadPct={downloadPercent * 100f:F1}%");
                    MainThreadInvoke(() => { OnTitleChanged?.Invoke(PublicTrackTitle); });
                    return;
                }
                ...
                if (downloadPercent >= PrescanAcceptThreshold)
                {
                    ...
                    if (best > 0)
                    {
                        ...
                        _hasRealLength = true;
                        _isFullyDownloaded = (downloadPercent >= 0.999f);
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Prescan accepted after download threshold. DownloadPct={downloadPercent * 100f:F1}%; TotalTime={TotalTime:F2}s");
                        MainThreadInvoke(() => { OnTitleChanged?.Invoke(PublicTrackTitle); });
                        return;
                    }
                }
                ...
                if (Time.realtimeSinceStartup - _watchLastProgressTime >= _watchProgressStallSeconds)
                {
                    ...
                    if (best > 0)
                    {
                        ...
                        _hasRealLength = true;
                        _isFullyDownloaded = (downloadPercent >= 0.999f);
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Prescan watch timeout/stall accepted. DownloadPct={downloadPercent * 100f:F1}%; TotalTime={TotalTime:F2}s");
                        MainThreadInvoke(() => { OnTitleChanged?.Invoke(PublicTrackTitle); });
                        return;
                    }
                }
                ...
                if (Time.realtimeSinceStartup - _watchStartTime >= _watchTimeoutSeconds)
                {
                    ...
                    if (best > 0)
                    {
                        ...
                        _hasRealLength = true;
                        _isFullyDownloaded = (downloadPercent >= 0.999f);
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] Prescan watch timeout reached. Using best estimate: TotalTime={TotalTime:F2}s, DownloadPct={downloadPercent * 100f:F1}%");
                        MainThreadInvoke(() => { OnTitleChanged?.Invoke(PublicTrackTitle); });
                        return;
                    }
                }
            ...
            if (lenBytes > 0 && !_hasRealLength)
            {
                ...
                double secs = Bass.ChannelBytes2Seconds(_stream, lenBytes);
                TotalTime = (float)secs;
                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] PollForLengthChange authoritative length -> {secs:F3}s (bytes={lenBytes})");
            }
        }
        catch (Exception ex)
        {
            ...
            if (string.IsNullOrEmpty(newTitle))
            {
                    ...
                    if (id3v2.TextFrames != null && id3v2.TextFrames.Count > 0)
                    {
                            ...
                            id3v2.TextFrames.TryGetValue("TT2", out title))     // Legacy title frame
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(title))
                            {
                                ...
                                if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] ID3v2 tags found: Title='{title}', Artist='{artist}'");
                            }
                        }
                    }
                }
                catch (Exception ex)
                {
                    ...
                    if (force) // Only log in forced updates to reduce spam
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"[StreamingClip] No ID3v2 tags or parsing failed: {ex.Message}");
                }
            }
    ...
    public static void TryInitBassOnce()
    {
