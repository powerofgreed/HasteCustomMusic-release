...
public class CustomMusicManager : MonoBehaviour
{
    ...
    public static PlayOrder CurrentPlayOrder
    {
        get
        {
            try
            {
                if (Enum.TryParse<PlayOrder>(PluginConfig.PlayOrder?.Value, out var order))
                    return order;
            }
            catch { }
            return PlayOrder.Sequential;
        }
        set { if (PluginConfig.PlayOrder != null) PluginConfig.PlayOrder.Value = value.ToString(); }
    }
    ...
    private void Awake()
    {
        if (_instance == null)
        {
            ...
            _disposed = false;
            InitializeHybridPlaylistFromConfig();
        }
    ...
    public static bool LoadLocalTracks(string directoryPath)
    {
        ...
        try
        {
            ...
            IsLocalPlaylistPreloaded = PluginConfig.PreloadEntirePlaylist.Value;
            ...
            var supported = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
{
    ...
    private void InitializeHybridPlaylistFromConfig()
    {
        try
        {
            // Load Hybrid playlist from config
            HybridTrackPaths.Clear();
            HybridTrackPaths.AddRange(PluginConfig.PlaylistConfig.HybridPlaylistPaths);
            CreateHybridPlaylistFromTracks();
            Debug.Log($"Initialized Hybrid playlist with {HybridTrackPaths.Count} tracks from config");
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error initializing Hybrid playlist from config: {ex}");
        }
    }
    ...
    public static void PlayDefaultTrack(int trackIndex)
    {
        ...
        var playlist = MusicDisplayPlugin.GetStoredDefaultPlaylist();
