...
public class MusicDisplayPlugin : BaseUnityPlugin
{
    ...
    public static LoaderPriority CurrentLoaderPriority
    {
        get
        {
            try
            {
                return PluginConfig.LoaderPriority?.Value ?? LoaderPriority.BassFirst;
            }
            catch
            {
                return LoaderPriority.BassFirst;
            }
        }
    }
    ...
    void Start()
    {
        ...
        _localMusicPath = PluginConfig.LocalMusicPath.Value;
        ...
        // Initialize playlist data from config
        InitializePlaylistData();

        // Handle forced custom playlist
        if (PluginConfig.ForceLocalPlaylist.Value)
        {
            _localMusicPath = GetValidatedMusicPath();
            StartCoroutine(LoadAndForceLocalPlaylist());
        }
        StreamingClip.OnTitleChanged += HandleStreamingTitleChanged;
        Debug.Log($"Audio Loader Priority: {CurrentLoaderPriority}");

        // Initialize volume master
    ...
    private void InitializePlaylistData()
    {
        try
        {
            // Load both playlists from their respective files
            PluginConfig.PlaylistConfig.LoadHybridPlaylistFromFile();
            PluginConfig.PlaylistConfig.LoadStreamsPlaylistFromFile();

            // Only initialize if files had tracks
            if (PluginConfig.PlaylistConfig.HybridPlaylistPaths.Count > 0)
            {
                CustomMusicManager.HybridTrackPaths.Clear();
                CustomMusicManager.HybridTrackPaths.AddRange(PluginConfig.PlaylistConfig.HybridPlaylistPaths);
                CustomMusicManager.CreateHybridPlaylistFromTracks();
            }

            if (PluginConfig.PlaylistConfig.StreamPlaylistPaths.Count > 0)
            {
                CustomMusicManager.StreamsTrackPaths.Clear();
                CustomMusicManager.StreamsTrackPaths.AddRange(PluginConfig.PlaylistConfig.StreamPlaylistPaths);
                CustomMusicManager.CreateStreamsPlaylistFromTracks();
            }

            Debug.Log($"Initialized playlists: {CustomMusicManager.HybridTrackPaths.Count} hybrid tracks, {CustomMusicManager.StreamsTrackPaths.Count} stream tracks");
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error initializing playlist data: {ex}");
        }
    }
    ...
    private void HandleResizing()
{
    ...
    {
        // Get configured path
        string configPath = PluginConfig.LocalMusicPath.Value;
        ...
        try
        {
            ...
            else
            {
                ...
                if (string.Equals(configPath, defaultPath, StringComparison.OrdinalIgnoreCase))
                {
                    try
                    {
                        Directory.CreateDirectory(defaultPath);
                        Debug.Log($"Created default directory: {defaultPath}");
                        return defaultPath;
                    }
                    catch (Exception ex)
                    {
                        Debug.LogWarning($"Failed to create default directory: {ex.Message}");
                    }
                }
            }
        }
        ...
        // Return the configured path even if it doesn't exist (user will see error when trying to load)
        return configPath;
    }
    ...
    private void ToggleBetweenDefaultAndCurrentTabPlaylist()
    {
        ...
        if (PluginConfig.ShowDebug.Value) Debug.Log($"Toggle: Current viewing={_viewingPlaylistType}, Target={targetPlaylistType}");

        // If we're currently viewing the target playlist, switch to view default
        if (_viewingPlaylistType == targetPlaylistType)
        {
            if (PluginConfig.ShowDebug.Value) Debug.Log("Toggling to view default playlist");
            SwitchToViewPlaylist(PlaylistType.Default);
        }
        // Otherwise, switch to view the target playlist
        else
        {
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Toggling to view {targetPlaylistType} playlist");
    ...
    private void SwitchToViewPlaylist(PlaylistType playlistType)
    {
        ...
        _lastClickedTrack = -1;
        GUI.changed = true;
        if (PluginConfig.ShowDebug.Value) Debug.Log($"Now viewing: {_viewingPlaylistType}");
    }
            ...
            if (Event.current.type == EventType.MouseDown && trackRect.Contains(Event.current.mousePosition))
            {
                ...
                if (displayIndex == _lastClickedTrack && now < _singleClickExpireAt)
                {
                    ...
                    else // Normal double-click: play track
                    {
                        ...
                        if (PluginConfig.ShowDebug.Value) Debug.Log($"Double-click: Playing track at display index {displayIndex} from viewed {_viewingPlaylistType}");
                            ...
                            viewedPlaylist.ShuffledOrder.Count == viewedPlaylist.TrackCount)
                        {
                            ...
                            if (PluginConfig.ShowDebug.Value) Debug.Log($"Shuffled playlist: display index {displayIndex} = track index {trackIndexToPlay}");
                        }
    ...
    private void ShuffleViewedPlaylist()
    {
        ...
        try
        {
            if (PlaylistManager.GetPlaylist(_viewingPlaylistType) is BasePlaylist viewedPlaylist)
            {
                ...
                if (PluginConfig.ShowDebug.Value) Debug.Log($"Shuffled {_viewingPlaylistType} playlist");
                GUI.changed = true;
            }
        }
        catch (Exception ex)
    ...
    private void StartSaveTimer()
    {
        ...
        if (PluginConfig.ShowDebug.Value) Debug.Log("Save timer started - click the countdown button to confirm");
    }
    ...
    private void StartLoadTimer()
    {
        ...
        _timerStartTime = Time.realtimeSinceStartup;
        if (PluginConfig.ShowDebug.Value) Debug.Log("Load timer started - click the countdown button to confirm");
    }
    ...
    private void CancelTimer()
    {
        _activeTimerType = "";
        _timerStartTime = 0f;
        if (PluginConfig.ShowDebug.Value) Debug.Log("Timer cancelled");
    }
    ...
    private void ExecuteSave()
    {
        try
        {
            ...
            SaveHybridPlaylistToFile();
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Saved {CustomMusicManager.HybridTrackPaths.Count} tracks to Hybrid playlist");
        }
        catch (Exception ex)
        {
    ...
    private void ExecuteLoad()
    {
        try
        {
            // Load Hybrid playlist using pause-resistant coroutine
            StartCoroutine(LoadHybridAfterFileCheck());
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Started loading Hybrid playlist");
        }
        catch (Exception ex)
        {
    ...
    private void LoadHybridPlaylistFromFileImmediate()
    {
        try
        {
            // Load from file first to check if we have tracks
            PluginConfig.PlaylistConfig.LoadHybridPlaylistFromFile();

            // Only proceed if we actually loaded tracks
            if (PluginConfig.PlaylistConfig.HybridPlaylistPaths.Count > 0)
            {
                // Clear the current Hybrid playlist synchronously
                CustomMusicManager.ClearPlaylist(PlaylistType.Hybrid);

                // Update the manager with loaded tracks
                CustomMusicManager.HybridTrackPaths.Clear();
                CustomMusicManager.HybridTrackPaths.AddRange(PluginConfig.PlaylistConfig.HybridPlaylistPaths);
                ...
                Debug.Log($"Immediate load: {CustomMusicManager.HybridTrackPaths.Count} tracks from Hybrid playlist");

                // Switch to view the loaded playlist
                SwitchToViewPlaylist(PlaylistType.Hybrid);
            }
            else
            {
                Debug.Log("Hybrid playlist file is empty - no changes made");
            }
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error in immediate Hybrid playlist load: {ex}");
        }
    }

    private void SaveHybridPlaylistToFile()
    {
        try
        {
            // Update the config list with current tracks
            PluginConfig.PlaylistConfig.HybridPlaylistPaths.Clear();
            PluginConfig.PlaylistConfig.HybridPlaylistPaths.AddRange(CustomMusicManager.HybridTrackPaths);

            // Save to file
            PluginConfig.PlaylistConfig.SaveHybridPlaylistToFile();
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error in SaveHybridPlaylistToFile: {ex}");
        }
    }

    private void LoadHybridPlaylistFromFile()
    {
        try
        {
            // Load from file first to check if we have tracks
            ...
            {
                Debug.Log("Hybrid playlist file is empty - no changes made");
                // Don't clear, don't switch view
            }
        }
        catch (Exception ex)
        {
            Debug.LogError($"Error in LoadHybridPlaylistFromFile: {ex}");
        }
    }

    private System.Collections.IEnumerator LoadHybridAfterFileCheck()
    {

        yield return StartCoroutine(ClearPlaylistCoroutine(PlaylistType.Hybrid));
        ...
        try
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Loaded {CustomMusicManager.HybridTrackPaths.Count} tracks from Hybrid playlist");
        ...
        {
            Debug.LogError($"Error in LoadHybridAfterFileCheck: {ex}");
        }
    }
    private void LoadStreamsPlaylistFromFile()
    {
        try
        {
            // Load from file first to check if we have tracks
            ...
            if (PluginConfig.PlaylistConfig.StreamPlaylistPaths.Count > 0)
            {
                StartCoroutine(LoadStreamsAfterFileCheck());
            }
            else
            {
                Debug.Log("Streams playlist file is empty - no changes made");
                // Don't clear, don't switch view
            }
        }
        catch (Exception ex)
    ...
    private System.Collections.IEnumerator LoadStreamsAfterClear()
    {
        yield return null; // Wait one frame for clear to complete
        ...
        try
        {
            ...
            Debug.Log($"Loaded {CustomMusicManager.StreamsTrackPaths.Count} streams from file");
        ...
        {
            Debug.LogError($"Error in LoadStreamsAfterClear: {ex}");
        }
    }
    private System.Collections.IEnumerator LoadStreamsAfterFileCheck()
    {

        yield return StartCoroutine(ClearPlaylistCoroutine(PlaylistType.Streams));
        ...
        try
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Loaded {CustomMusicManager.StreamsTrackPaths.Count} streams from file");
        ...
        {
            Debug.LogError($"Error in LoadStreamsAfterFileCheck: {ex}");
        }
    }
    ...
    private System.Collections.IEnumerator ClearPlaylistCoroutine(PlaylistType playlistType)
    {
        // Use a small realtime delay to ensure any ongoing operations complete
        yield return new WaitForSecondsRealtime(0.05f);

        // Clear the playlist synchronously (this doesn't need to be a coroutine)
        CustomMusicManager.ClearPlaylist(playlistType);

        // Wait one more frame to ensure clear is processed
        yield return new WaitForSecondsRealtime(0.05f);

        Debug.Log($"Cleared {playlistType} playlist");
    }
    ...
    private void AddToHybridPlaylist(string path)
    {
        ...
        try
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Added track to Hybrid playlist: {displayName}");
    ...
    private void PlayHybridTrackDirect(string path)
    {
        ...
        try
        {
            ...
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Playing Hybrid track: {path}");
        }
        catch (Exception ex)
        {
    ...
    private System.Collections.IEnumerator SwitchToStreamsAfterLoad()
    {
        ...
        {
            SwitchToViewPlaylist(PlaylistType.Streams);
            if (PluginConfig.ShowDebug.Value) Debug.Log($"Switched to view streams playlist with {CustomMusicManager.StreamsTrackPaths.Count} tracks");
        }
    }
    void Update()
    {
    ...
    private void CalculateUIScale()
    {
        ...
        if (PluginConfig.ShowDebug.Value && Time.frameCount % 60 == 0)
        {
